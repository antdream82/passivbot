...
- **Live side-specific approved coins enforcement** - Live forager selection now carries side-specific new-entry eligibility into the Rust orchestrator and blocks unapproved-side entry orders as a final Python safety check, preventing symbols approved only for short from opening long entries, and vice versa.
